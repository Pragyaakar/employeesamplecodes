...
function suiteletPDFmultiSecondaryBook(request, response)
{
   ...
   // for (var z = 0; z < vendor_a.length; z++) 
	...
	strName += "</tr>";*/
	 ...
	 var columns1 = new Array();
		...
		filters1.push(new nlobjSearchFilter('accountingbook', null, 'anyof', '2'));
	...
	if (searchResults1 != null && searchResults1 != '')
	{
		for (var i1 = 0;  i1 < searchResults1.length; i1++) 
		{
			...
			var createdFrom1 = searchResults1[i1].getText("createdfrom","transaction");
			
			var ExchRate1 = searchResults1[i1].getValue("exchangerate");
			
	
			if(createdFrom1 != null && createdFrom1 != undefined && createdFrom1 !='' && createdFrom1.indexOf('#') != -1)
			{
				 var createDoc = createdFrom1.substring(createdFrom1.indexOf('#')+1, createdFrom1.length);
				
					RevalMap[createDoc.trim()]=TranAmt1;
	...
	filters.push(new nlobjSearchFilter('accountingbook', null, 'anyof', '2'));
	...
	if (searchResults != null && searchResults != '')
	{
		...
		// nlapiLogExecution('DEBUG', 'ScheduleTest', 'search result length = ' + length);
			...
			for (var i = 0;  i < length; i++) 
			{
				...
				RevolRate = parseFloat(RevalMap[DocNo]);

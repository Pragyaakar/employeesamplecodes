...
function PurchaseOrderLineUpdation(type, form, request)
{
	...
	var recordType = nlapiGetRecordType();
		...
		var expOrderArr =[];
		var ordArr=[];
		if(ItemlineCount > 0 && (CreateFrom ==null || CreateFrom ==undefined || CreateFrom ==''))//&& (CreateFrom ==null || CreateFrom ==undefined || CreateFrom =='')
		{
		...
		else if(ExplineCount > 0 && (CreateFrom ==null || CreateFrom ==undefined || CreateFrom =='')){

			for (var a=1; a<=ExplineCount ;a++)
			{
				var line_order =nlapiGetLineItemValue('expense','custcol_cls_bill_order',a);
				if(line_order !=null && line_order!='' && line_order !=undefined)
				{
					ordArr.push(line_order);
				}
			...
			if(searchResultItem != null)
			{
				...
				//nlapiLogExecution('DEBUG','Search Value','billMapSetArr ='+billMapSetArr);
					...
					for (var m=1; m<=ExplineCount; m++)
					{
						var OrdField =nlapiGetLineItemValue('expense','custcol_cls_bill_order',m);
						var OrdFieldLine =nlapiGetLineItemValue('expense','line',m);
						    ...
						    if(OrdField !=null && OrdField !='' && OrdField !=undefined)
							{
						    	...
						    	nlapiSetLineItemValue('expense','custcol_order_number',m,itmOrderNumArr[OrdField]);
						    	nlapiSetLineItemValue('expense','custcol_entity_name',m,itmEntityArr[OrdField]);
							}
					}

...
function userEventVbillCreditLineDataBlank(type)
{
	...
	var recordType = nlapiGetRecordType();
		...
		var searchId ='customsearch665';
		
		var RMAcreate =irObj.getFieldText('createdfrom');
		nlapiLogExecution('DEBUG','Search Value','RMAcreate :'+RMAcreate);
		
		if(RMAcreate.substr(0,27) =='Vendor Return Authorization')
		{
			...
			var UsrChk = nlapiGetUser();
				...
				if(resultSet !=null && resultSet !='' && resultSet != undefined)
				{
					...
					for(i1=0;i1<resultSet.length;i1++)
				    {
						...
						var FromOffromCreat =  resultSet[i1].getValue("createdfrom","appliedToTransaction");
						var FromOffromCreatTxt =  resultSet[i1].getText("createdfrom","appliedToTransaction");
						
						nlapiLogExecution('DEBUG','Search Value','FromOffromCreatTxt :'+FromOffromCreatTxt);

					
						if(FromOffromCreatTxt.substr(0,11) =='Sales Order')
						{
							       ...
							       if(lineid == lineNumArr[m] )
							    	{
										 ...
										 nlapiLogExecution('DEBUG','Search Value','Line num set ='+i);
							    	}
							       else
							       {
											...
											nlapiLogExecution('DEBUG','Search Value','Line num Not set ='+i);
							       }

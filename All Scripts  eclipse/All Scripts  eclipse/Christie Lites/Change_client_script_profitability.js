...
function email_excel_report(){
	...
	var emailSubj = nlapiGetFieldValue('custpage_subject');
	var emailRecip = nlapiGetFieldValue('custpage_recipient');
	var emailMsg = nlapiGetFieldValue('custpage_message');
	
	if(!isEmpty(emailRecip))
	{
		...
		if (!isEmpty(endDate)) {
			...
			if (startDate_obj <= enddate_obj) {
				...
				if(!isEmpty(salesRep)){
					...
					var parameters = '&stdt='+startDate+'&eddt='+endDate+'&srep='+salesRep+'&emsub='+emailSubj+'&emrecp='+emailRecip+'&emsg='+emailMsg;//+'&allrep='+AllSalesRep;
					if(!isEmpty(customer)){
						parameters += '&cus='+customer;
					}
					if(!isEmpty(salesOrder)){
						...
						var url = nlapiResolveURL('SUITELET','customscript_suite_sched_n_email_trigger','customdeploy_suite_sched_n_email_trigger');
	...
	{
		alert('Please Enter Email Recipient');
	}

...
//================= Set Parent Asset =========================		
		...
		nlapiLogExecution('DEBUG', 'b4 submit', "  OObj  ==" + Obj+' tranResultSet2'+tranResultSet2);
		 	...
		 	if(tranResultSet1!=null && tranResultSet1!='' && tranResultSet1!=undefined)
		 	{
		 	...
		 	else if(tranResultSet2!=null && tranResultSet2!='' && tranResultSet2!=undefined)
		 	{
...
//====================== ENd ===============================		
		...
		var ParentAsset = relatedAsset;  //entity
	 	nlapiLogExecution('DEBUG', 'aftr submit', "  ParentAsset  ==" + ParentAsset);
		 	...
		 	if(ParentAsset != null && AssetProject!=null && ParentTransaction!=null)
	        {
			 	...
			 	if(tranResultSet!=null && tranResultSet!='' && tranResultSet!=undefined)
			 	{
			 		...
			 		{
				 		var Acc = tranResultSet[m1].getValue('account');
				 		AccArray.push(Acc);
			 		}
			 		...
			 		// for (var m=0;m<jsonArray3.length;m++)
			 		 {
			 			for(var i=0;i<tranResultSet.length;i++)
						{
				 			...
				 			var AmtFor = tranResultSet[i].getValue('fxamount');
				 			nlapiLogExecution('DEBUG', 'aftr submit', "  AmtFor  ==" + AmtFor);
				 			
				 			if(AssetAcc ==jsonArray3[m])
			 				{
...
function userEvntToBeforeSubmitFAmAsset(type, form, request)
{
	 try
	  {
		   var itemArray = new Array();
		...
		var ParentAsset = recObj.getFieldValue('custrecord_assetparent');  //entity
	 	nlapiLogExecution('DEBUG', 'b4 submit', "  ParentAsset  ==" + ParentAsset);
	 	
	 	var AssetDescription = recObj.getFieldValue('custrecord_assetdescr');  //entity
	 	nlapiLogExecution('DEBUG', 'b4 submit', "  AssetDescription  ==" + AssetDescription);
	 	
		
		var AssetProject = recObj.getFieldValue('custrecord_assetproject');  //entity
		 	nlapiLogExecution('DEBUG', 'b4 submit', "  AssetProject  ==" + AssetProject);
		 	
		 var ParentTransaction = recObj.getFieldValue('custrecord_assetsourcetrn');
		 	nlapiLogExecution('DEBUG', 'b4 submit', "  ParentTransaction  ==" + ParentTransaction);
		 	
		
			var Obj =nlapiLoadRecord('vendorbill',ParentTransaction);
			
			var tranResultSet =Obj.getLineItemCount('item');
			nlapiLogExecution('DEBUG', 'b4 submit', "  OObj  ==" + Obj+' tranResultSet'+tranResultSet);
			 
			 	if(tranResultSet!=null && tranResultSet!='' && tranResultSet!=undefined)
			 	{
			 		
			     	for(var i=1;i<=tranResultSet;i++)
						{
				 			var memo = Obj.getLineItemValue('item','description',i);
				 			nlapiLogExecution('DEBUG', 'b4 submit', "  memo  ==" + memo);
				 			
				 			var relatedAsset = Obj.getLineItemValue('item','custcol_far_trn_relatedasset',i);
				 			nlapiLogExecution('DEBUG', 'b4 submit', "  relatedAsset  ==" + relatedAsset);
				 		
				 			if(AssetDescription == memo)
			 				{
				 				nlapiLogExecution('DEBUG', 'b4 submit', "  relatedAsset  Setting==");
				 				nlapiSetFieldValue('custrecord_assetparent',relatedAsset);
								nlapiSetFieldValue('custrecord_assetdepractive','2');
								 nlapiSubmitField(recordType,recordId,'custrecord_assetdepractive','2');
				 				 nlapiSubmitField(recordType,recordId,'custrecord_assetparent',relatedAsset);
			 				}
						} 
	  ...
	  {
	    nlapiLogExecution("DEBUG","In Create Function","Before Load Wala=="+e );
	  }
}

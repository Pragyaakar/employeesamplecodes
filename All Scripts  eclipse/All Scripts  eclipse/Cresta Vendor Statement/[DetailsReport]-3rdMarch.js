...
//================================Subsidiary Address==================================					
 ...
 //=========================================Vendor Address=====================================
  //  var vendor_deatil = nlapiLookupField('vendor',vendor_a,'entityid');
	...
	var strName ="";
	// nlapiLogExecution('DEBUG','before submit','CurrencyListArr =='+CurrencyListArr);
                ...
                for (var z = 0; z < CurrencyListArr.length; z++) 
					{
						...
						var transCurr =CurrencyListArr[z];
						vendor_currency=CurrencyListArr1[z];
						// nlapiLogExecution('DEBUG','before submit','vendor_currency =='+vendor_currency);
						...
						//if (vendor != null && vendor != '' && vendor != undefined) 
						{
							...
							//	nlapiLogExecution('DEBUG', 'searchVendorTransactions', 'custCurrency = ' + custCurrency);
								...
								var openingBalance = getOpeningBalance(vendor, locationname, custCreatedDate, opBalanceEndDate,transCurr);
								var openingBalanceFx = getOpeningBalance1(vendor, locationname, custCreatedDate, opBalanceEndDate,transCurr);
							/*	
	...
	strName += "</tr>";
	strName += "<tr>";
	strName += "<td  width = \"50\%\" colspan=\"6\" align=\"left\"><b>Payment Terms :</b>&nbsp;"+nlapiEscapeXML(vendor_terms)+"</td>";	
	strName += "</tr>";
	strName += "<tr>";
	strName += "<td  width = \"50\%\" colspan=\"6\" align=\"left\" margin-bottom=\"10px\"><b>Currency :</b>&nbsp;"+nlapiEscapeXML(vendor_currency)+"</td>";	
	strName += "</tr>";
	strName += "</table>";
	...
	strName += "<td border-top=\"solid 1px black\" border-bottom=\"solid 1px black\" width = \"20\%\" border-left=\"solid 1px black\"  align=\"center\" border-collapse=\"collapse\" padding-left=\"2em\" padding-right=\"2em\" style=\" white-space: nowrap;\"><b>Secondary Currency</b></td>";	
	...
	if(transCurr != null && transCurr != undefined && transCurr !='')
	{
		filters.push(new nlobjSearchFilter('currency', null, 'anyof', transCurr));
	}
			...
			//	nlapiLogExecution('DEBUG', 'ScheduleTest', 'foreignAmount = ' + foreignAmount);
				...
				if ( i >= 900) {
				 ...
				 strName += "<tr>";
					...
					{
						strName += "<td  width = \"30\%\"  padding-left=\"2em\" padding-right=\"2em\" border-bottom=\"solid 1px black\" border-left=\"solid 1px black\"  align=\"center\" style=\" white-space: nowrap;\" border-collapse=\"collapse\">"+date+"</td>";//convertDate1(date,dateformat)
					}
					else 
					{
						date =' ';
					...
					if (date != null && date != '' && date != undefined) 
					{
						strName += "<td  width = \"20\%\"  padding-left=\"2em\" padding-right=\"2em\" border-bottom=\"solid 1px black\" border-left=\"solid 1px black\"  align=\"center\" border-collapse=\"collapse\">"+date+"</td>";//convertDate1(date,dateformat)
					}
					else 
					{
						date =' ';
						strName += "<td  width = \"20\%\"  padding-left=\"2em\" padding-right=\"2em\" border-bottom=\"solid 1px black\" border-left=\"solid 1px black\"  align=\"center\" style=\" white-space: nowrap;\" border-collapse=\"collapse\">"+date+"</td>";//convertDate1(date,dateformat)
					}
					...
					if (type == 'Payment') 
					{
						...
						else
						{
							...
							// nlapiLogExecution('DEBUG', 'ScheduleTest payment', 'Effective Date = ' + today1);
					...
					if (Status == 'open') 
					{
						...
						{
							// nlapiLogExecution('DEBUG', 'ScheduleTest', 'currency = ' + currency);
							// nlapiLogExecution('DEBUG', 'ScheduleTest open', 'Effective Date = ' + today1);
...
function getOpeningBalance(vendor, locationname, openingBalanceDate, opBalanceEndDate,transCurr)
{
		/*		nlapiLogExecution('DEBUG', 'getOpeningBalance', 'vendor = ' + vendor);
			...
			*/	
				...
				totalAmount = parseFloat(totalAmount);
				 ...
				 var filters = new Array();
				 filters[0] = new nlobjSearchFilter ('entity', null, 'anyof',vendor);//transCurr
				 filters[1] = new nlobjSearchFilter ('trandate', null, 'onorbefore', opBalanceEndDate);
				
				 if(transCurr != null && transCurr != undefined && transCurr !='')
					{
				       filters[2] = new nlobjSearchFilter ('currency', null, 'anyof',transCurr);
					}
				var searchresults = nlapiSearchRecord('transaction','customsearch_cresta_vendorstatement',filters,columns)
...
function getOpeningBalance1(vendor, locationname, openingBalanceDate, opBalanceEndDate,transCurr)
{
		/*		nlapiLogExecution('DEBUG', 'getOpeningBalance', 'vendor = ' + vendor);
				nlapiLogExecution('DEBUG', 'getOpeningBalance', 'locationname = ' + locationname);
				nlapiLogExecution('DEBUG', 'getOpeningBalance', 'openingBalanceDate = ' + openingBalanceDate);

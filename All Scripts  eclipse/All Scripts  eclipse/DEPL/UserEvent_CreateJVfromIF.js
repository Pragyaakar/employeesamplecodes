...
function userEventIFtoJVAfterSubmit(type)
{
			...
			for(var i=1;i<=count;i++)
		    {
				...
				var projCategory =iarec.getLineItemValue('item','custcol_prj_categry',i); //	custcol_prj_categry
				  ...
				  else
				  {
				    ...
				    nlapiLogExecution('DEBUG','amount','clas----'+clas);
					nlapiLogExecution('DEBUG','amount','dept---'+dept);
					nlapiLogExecution('DEBUG','amount','route---'+route);
					nlapiLogExecution('DEBUG','amount','loc---'+loc);
				
				}	
		    }
			nlapiLogExecution('DEBUG','amount','clasArr----'+clasArr);
			nlapiLogExecution('DEBUG','amount','deptArr---'+deptArr);
			nlapiLogExecution('DEBUG','amount','routeArr---'+routeArr);
			nlapiLogExecution('DEBUG','amount','locArr---'+locArr);
	...
	function generateTransaction(internalId,amtArr,amtArr1,date,createdFrom,accArr,accArr1,clasArr,deptArr,routeArr,name,locArr)
	{
	 ...
	 var memo ='Cost Of Sales';
	     if(accArr.length > 0)
		  {
	    	 nlapiLogExecution('DEBUG','amount','inside accArr.length > 0 ---');
			...
			if(accArr[k-1] !=null && accArr[k-1] != undefined && accArr[k-1] !='' && accArr[k-1] =='2098')  // && ipAcc=='116'
			  {
				...
				if(accArr[k-1] !=null && accArr[k-1] != undefined && accArr[k-1] !='' && accArr[k-1] =='2098') 
				{
					jeRec.setCurrentLineItemValue('line', 'account',  accArr[k-1]);
				}
				if(amtArr[k-1] !=null && amtArr[k-1] != undefined && amtArr[k-1] !='') 
				{
		...
		var ID = nlapiSubmitRecord(jeRec);	
	     ...
	     if(accArr1.length > 0)
		  {
	    	 nlapiLogExecution('DEBUG','amount','inside accArr1.length > 0 ---');

...
function updtEmpQta(request, response)
{
	...
	Start_year= request.getParameter('strt_year');//pre_qota
	nlapiLogExecution('DEBUG', 'aftr submit', "  Start_year  ==" + Start_year);
  
    var qotarecord= request.getParameter('pre_qota');//pre_qota
	nlapiLogExecution('DEBUG', 'aftr submit', "  qotarecord  ==" + qotarecord);
...
// findCR(salesRep,Year,recId);
	...
	/*if(transItemSearch.length >=1000)
		{*/
			var params = new Array();
			params['custscript_sales_rep']=salesRep;
			params['custscript_qtaid']=scrhObjCr;
			params['custscript_strt_year']=Start_year;
			params['custscript_mnth']=Start_month;
			params['custscript_user_id']=currentUserID;
            params['custscript_quotaamt']=qotarecord;

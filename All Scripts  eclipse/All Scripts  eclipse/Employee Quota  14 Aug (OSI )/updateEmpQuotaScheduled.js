...
function empQtaFrmSutlet(type) 
{
	...
	var valueQuotaAmt = nlapiGetContext().getSetting('SCRIPT','custscript_emp_quota_creation');	
	nlapiLogExecution('DEBUG', 'valueQuotaAmt', valueQuotaAmt);
	...
	nlapiLogExecution('AUDIT','empQtaFrmSutlet','itemArray'+ JSON.stringify(itemArray));
/*	
	var lenOf = itemArray[0];
	
	  for(var Ch=0; 0<parseFloat(lenOf);Ch++)
	  {
		    		 nlapiLogExecution('DEBUG', 'Value Setting', "  quotaAmt Setting  ==" +lenOf);
	    		  
	    		var GP =0;
	    		var Carybalance =valueQuotaAmt;
	    		// nlapiSubmitField('customrecord449',scrhObjCr,'custrecord8',Ch+1);
	    		nlapiSubmitField('customrecord449',scrhObjCr,'custrecord10',GP);
	    		nlapiSubmitField('customrecord449',scrhObjCr,'custrecord12',Carybalance);
	...
	for(var k=0;k<itemArray.length;k++)
	 {
			...
			nlapiLogExecution('AUDIT','empQtaFrmSutlet', 'jsonArray'+ jsonArray.length);
			   ...
			   { 	   
      		    ...
      		    // nlapiLogExecution('Debug','mon1 < Start_month ', 'mon1=='+mon1+'=Start_month=='+Start_month);
      	    	//     nlapiLogExecution('Debug','mon1 < Start_month ', 'B4 itemArray[0] > mon1 '+itemArray[0] +':::'+mon1);

...
function afterSubmitRecord_vendorAdv(type)
{
    /*  On after submit:
    ...
    */
	...
	try
	{
		...
		{
			var checkid = nlapiGetRecordId()
			nlapiLogExecution('DEBUG', 'afterSubmitRecord check ', " checkid in after submit ==" + checkid)
			...
			var purchaseOrderNo = o_checkobj.getFieldValue('custbody_purchaseorder_no');
			nlapiLogExecution('DEBUG', 'afterSubmitRecord check', " purchaseOrderNo " + purchaseOrderNo)
			...
			if (o_checkobj != null && o_checkobj != '' && o_checkobj != undefined) 
			{
				...
				var b_Adavncetoved = o_checkobj.getFieldValue('custbody_vendor_advance')
				nlapiLogExecution('DEBUG', 'afterSubmitRecord check', " advance to vendor " + b_Adavncetoved)
				...
				if (b_Adavncetoved == 'T') 
				{
						date = o_checkobj.getFieldValue('trandate');
						payee = o_checkobj.getFieldValue('entity');
						nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "payee in advance check" + payee)
						if(payee !=null && payee != undefined)
                         {
                           account = nlapiLookupField('vendor',payee,'payablesaccount');
                           nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "account in advance check" + account)
                         }
						subsidiary = o_checkobj.getFieldValue('subsidiary');
						var nexus =o_checkobj.getFieldValue('custbody_nexus');//custbody_nexus
						...
						var creditAmt;
						var secRate ;
					var setID =	createBillCreditRecord(purchaseOrderNo,date,payee,acctArrayExp,rateArrayExp,taxArrayExp,itemArrayItm,qtyArrayItm,rateArrayItm,taxArrayItm,o_checkobj,checkid,
								deptArrayExp,classArrayExp,projectArrayExp,deptArrayItm,classArrayItm,projectArrayItm,subsidiary,totalAmount,locationArrayItem,locationArrayExp,tdsValue,creditAmt,amtTot,nexus,account);
				}
				
				
				nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "setID" + setID);
...
// BEGIN FUNCTION ===================================================
{
 // == FUNCTION FOR SEARCHING SUBSIDIARY GOLBAL PARAMETER ==
    ...
    function createBillCreditRecord(purchaseOrderNo,date,payee,acctArrayExp,rateArrayExp,taxArrayExp,itemArrayItm,qtyArrayItm,rateArrayItm,taxArrayItm,o_checkobj,checkid,
    		deptArrayExp,classArrayExp,projectArrayExp,deptArrayItm,classArrayItm,projectArrayItm,subsidiary,totalAmount,locationArrayItem,locationArrayExp,tdsValue,creditAmt,amtTot,nexus,account)
    {	
    	var tdsPayable = 0.00;
    	var payableAmt = 0.00;
    	...
    	var o_cstmTran_obj = nlapiCreateRecord('customtransaction_vendor_advc_payment');
    	...
    	o_cstmTran_obj.setFieldValue('custbody_purchaseorder_no',purchaseOrderNo);
    	
    	nlapiLogExecution('DEBUG', 'set values', "checkid" + checkid+"date"+date+"subsidiary"+subsidiary+'payee=='+payee);
    	
//=====================================Credit Entry=====================================    
    	o_cstmTran_obj.selectNewLineItem('line');
    	o_cstmTran_obj.setCurrentLineItemValue('line', 'account', '1174');
    	//o_cstmTran_obj.setCurrentLineItemValue('line', 'credit', amtTot);
    	//o_cstmTran_obj.setCurrentLineItemValue('line', 'entity', payee);
    	
    	o_cstmTran_obj.setCurrentLineItemValue('line', 'department', deptArrayItm);
    	nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "Department Done" + deptArrayItm);
    	
    	o_cstmTran_obj.setCurrentLineItemValue('line','class', classArrayItm);
    	nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "Class Done" + classArrayItm);
    	
    	o_cstmTran_obj.setCurrentLineItemValue('line', 'location', locationArrayExp);
    	nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "Location Done" + locationArrayExp);
    	
    	o_cstmTran_obj.setCurrentLineItemValue('line', 'credit', totalAmount);
    	nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "totalAmount credit" + totalAmount);
    	o_cstmTran_obj.commitLineItem('line');
    	
//=====================================Debit Entry=====================================     	
    	o_cstmTran_obj.selectNewLineItem('line');
    	o_cstmTran_obj.setCurrentLineItemValue('line', 'account', account);
    	...
    	o_cstmTran_obj.setCurrentLineItemValue('line', 'debit', totalAmount);
    	nlapiLogExecution('DEBUG', 'afterSubmitRecord check', "totalAmount debit" + totalAmount);
    	o_cstmTran_obj.commitLineItem('line');

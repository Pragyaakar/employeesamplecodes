		...
		var remPoArr=[];
          ...
          var MaterialRemQtyArr=[];
			...
			for(var i=1;i<=PRlinecount;i++)
			{
              ...
              var invCreated = irObj.getLineItemValue('item','custcol_inventory_adjustment_id',i);
				// nlapiLogExecution('DEBUG', 'aftr submit', "  ISrequireQty  ==" + ISrequireQty);
              var MaterialRemQty = irObj.getLineItemValue('item','custcol_remain_qty_of_adjustment',i);
              if(MaterialRemQty!=null && MaterialRemQty!='' && MaterialRemQty!=undefined )
				 {
              ...
              if(invCreated!=null && invCreated!='' && invCreated !=undefined )
				 {
			 ...
			 if( ISrequireQtyArr.length >=1 && InvAdArr.length !=ISrequireQtyArr.length && MaterialRemQtyArr.length !=InvAdArr.length ) //status == '2' &&  commented for pradnyas testing
	         {
		  		    form.setScript('customscript_client_location_acc_redirec');
		  			form.addButton('custpage_button1','Material Issue','clientScriptMIredirect();');
	         }
          else if(InvAdArr.length ==PRlinecount)
           {

...
function clientNegativeInventoryControl(type, name, linenum)
{
	 ...
	 if(type=='inventory' && name == 'adjustqtyby') 
     {
		...
		/*    if((item!=null && item != '' )&&( location!=null && location != ''))
		 ...
		 avail_quant = nlapiGetCurrentLineItemValue('inventory','quantityonhand');
			// alert('AdjustQty is'+AdjustQty);
...
function clientSaveRecord()
{
	 if(nlapiGetContext().getExecutionContext() != 'userinterface')
	 {
			...
			{
				var adjQty=nlapiGetLineItemValue('inventory','adjustqtyby',p);
				var onHandQty=nlapiGetLineItemValue('inventory','quantityonhand',p);//quantityonhand
				 nlapiLogExecution('DEBUG', 'item', 'adjQty ' + adjQty);
				 nlapiLogExecution('DEBUG', 'item', 'onHandQty ' + onHandQty);
				var newQty = parseFloat(onHandQty)+parseFloat(adjQty);
				nlapiLogExecution('DEBUG', 'item', 'newQty ' + newQty);
				alert('newQty is'+newQty)

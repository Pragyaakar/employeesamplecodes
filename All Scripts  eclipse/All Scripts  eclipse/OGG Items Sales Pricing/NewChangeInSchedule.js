...
function  findSupplierQtyBreak(itemNames,categoryProduct,markUpJson,DiscountMap,priceLevel)
{
	...
	if (UniSearch != null)
	{
		for(var jk=0;jk<UniSearch.length;jk++)
		{
			...
			var itemTypeInRec = UniSearch[jk].getValue('custrecord_item_type');
		    	    	...
		    	    	//  nlapiLogExecution('DEBUG','SCH_ItemsSalesPricingCalc', ":= :- RRP0:-"+RRP0);
		    	    	  ...
		    	    	  if(Qty==0 || Qty==1 )
		    	    	  {
									   ...
									   if(priceLevel  != '9' && priceLevel != '10' )
					    			    {
										   ...
										   else
										   {
									         ...
									         RRPset[intId]=parseFloat(RRP0).toFixed(2);
							...
							catch(e)
							{
							 ...
							 // nlapiSubmitField('customrecord_sv_supplier_qty_break',intId,'custrecord_rec_rrp',RRP0);
			    	    }			

function User_BeforeSubmitFreightUpdate(type, form, request)
{
  try
  {
			...
			// nlapiLogExecution('DEBUG','findSupplierQtyBreak()', " := Vendor:-  "+Count);
			 ...
			 for(var m=1;m<=Count;m++)
			 {
				...
				// nlapiLogExecution('DEBUG','findSupplierQtyBreak()', " := isPref:-  "+isPref);
                 ...
                 if(VendPref !=null && VendPref!='' && VendPref!=undefined)
                 {
                       var newsupp= nlapiLoadRecord('vendor',VendPref);
                     //=========================================================  
                   	var freeThreshold = newsupp.getFieldValue('custentity_free_freight_threshold');
            		var minDeliDays = newsupp.getFieldValue('custentity_minimum_del_time');
            		var maxDeliDays = newsupp.getFieldValue('custentity_maximum_del_time');
            		var BasicCharge = newsupp.getFieldValue('custentity_basic_charge');
            		var CostPerKg = newsupp.getFieldValue('custentity_cost_kg');
            		var InitialCartanCost = newsupp.getFieldValue('custentity_cost_first_carton');
            		var AddCartanCost = newsupp.getFieldValue('custentity_cost_additional_carton');
            		var MaxKgInCarton = newsupp.getFieldValue('custentity_max_kg');
            		// var productWeght = itemObj.getFieldValue('weight');
            		...
            		filters1[0] = new nlobjSearchFilter('custrecord_sv_supplier', null,'anyof',VendPref);
            		filters1[1] = new nlobjSearchFilter('custrecord_sv_item', null,'anyof',recordId);
                    ...
                    //====================================================================   
                       //  nlapiSubmitRecord(newsupp,true);
                      nlapiLogExecution('DEBUG','findSupplierQtyBreak()', " := Updated Custom record:-  "+intId);
                   }

function afterSupplierSubmitFreightChargeUpdate(type, form, request)
{
  try
  {
	  var recordId = nlapiGetRecordId();
		...
		// if(myRole =='1972')
		{
			 nlapiLogExecution('DEBUG','Supplier Freight Cost', " := myRole:-  "+myRole);
			 
			 var myParams = [];
				
				myParams.custscript_freethreshld = freeThreshold;
				myParams.custscript_mindeldays = minDeliDays;
				myParams.custscript_maxdeldays = maxDeliDays;
				myParams.custscript_basiccharge = BasicCharge;
				myParams.custscript_costperkg = CostPerKg;
				myParams.custscript_initialcartcost = InitialCartanCost;
				myParams.custscript_addcartcost = AddCartanCost;
				myParams.custscript_maxkgincart = MaxKgInCarton;
                myParams.custscript_rec_id = recordId;
				nlapiScheduleScript("customscript_supp_freight_cost_update","customdeploy_supp_freight_cost_update", myParams);

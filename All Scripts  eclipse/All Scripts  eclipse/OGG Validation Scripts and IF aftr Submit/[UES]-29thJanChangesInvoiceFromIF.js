	 ...
	 try
	  {
		...
		if(status === 'C')
    	{
			if(createdFrom !=null && createdFrom != undefined && createdFrom !='')
			{
				...
				{
				    var invoiceID = createInvoiceFunction(customer,recordId,ifDocNo,date,itemArr,qtyArr,rateArr,TrackNoArr,locArr,bodyLoc,createdFrom,invVal,ShippMethod,ShippCost);
				}
		...
		// var recipientLook = nlapiLookupField('invoice',invoiceID,'email');
		 ...
		 if(emailRecipient !=null && emailRecipient !='' && emailRecipient!=undefined)
		 {
...
function createInvoiceFunction(customer,recordId,ifDocNo,date,itemArr,qtyArr,rateArr,TrackNoArr,locArr,bodyLoc,createdFrom,invVal,ShippMethod,ShippCost)
{

	   ...
	   var form = nlapiCreateForm("ROHL Order Requisition");
		...
		filters[0] = new nlobjSearchFilter('subsidiary', null, 'is', subsidary);
		  var searchResults = nlapiSearchRecord('vendor','customsearch_vendor_search_namewise',filters,[new nlobjSearchColumn('altname')]);
		   ...
		   ItemSublist.addField('custpage_class','select', 'CLASS','class').setDisplayType('disabled');
	...
	else
	{
		if (request.getMethod() == 'POST') 
		{
		 ...
		 var vendorName=request.getParameter('custpage_vendorname');
		  ...
		  createPurchaseOrderForPR(lineNum,lineNumArr,vendorName,amtArr,subsiArr,lastPurchaseArr,descripArr,classArr,departArr,LocArr,itemArr,quanArr,unitArr,tranidArr,hsncodeArr);
		  response.writePage( form );
		 }
	}
}
...
function createPurchaseOrderForPR(lineNum,lineNumArr,vendorName,amtArr,subsiArr,lastPurchaseArr,descripArr,classArr,departArr,LocArr,itemArr,quanArr,unitArr,tranidArr,hsncodeArr)	
{	
	...
	*/   
	...
	for(var l=1;l<=lineNum;l++)
	{
				...
				nlapiLogExecution("DEBUG","In Create Function","amount in for loop=="+amtArr[l-1]);
                  ...
                  record.setCurrentLineItemValue('item', 'custcol_pr_quantity', quanArr[l-1]);                              
			      nlapiLogExecution("DEBUG","In Create Function"," quantity done=="+quanArr[l-1]);

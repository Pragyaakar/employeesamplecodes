		...
		for(var jk=1;jk<=suiteletCount;jk++)
		{
			...
			for (var ah=1; ah<=itemCount;ah++)
			{
				...
				if(ah == lineNumSuite)
				{
					...
					/*
						   if(parseFloat(minusQty1) <=parseFloat(InvQuantity)  && tempStore == 0)
							{
						   ...
						   else if(parseFloat(minusQty1 > InvQuantity) && (tempStore > 0) && (InvQuantity > parseFloat(minusQty1-parseFloat(tempStore))))
						   {
...
//====================================== Setting Inventory Details =====================================================
			...
			for(var r = 0;r < inventorydetailSearch.length;r++)
		    {
				...
				nlapiLogExecution('DEBUG', 'Acct', 'InvQuantity ==' +InvQuantity);
				   ...
				   /*else if((parseFloat(qtyArray[x-1] > InvQuantity))&& (parseFloat(tempStore > 0)) && (InvQuantity > parseFloat(qtyArray[x-1]-parseFloat(tempStore))))
				   {
					   nlapiLogExecution('DEBUG', 'Acct', 'condition 3 ==');
					   InvQuantity = parseFloat(qtyArray[x-1]-parseFloat(tempStore));
					   tempStore += parseFloat(InvQuantity);
					   nlapiLogExecution('DEBUG', 'Acct', 'condition 3 =:tempStore='+tempStore);
				   }
				   else if(parseFloat(qtyArray[x-1] > InvQuantity) && parseFloat(tempStore > 0) && (InvQuantity < parseFloat(qtyArray[x-1]-parseFloat(tempStore))))
				   {
					   nlapiLogExecution('DEBUG', 'Acct', 'condition 4 ==');
					   InvQuantity = parseFloat(InvQuantity);
					   tempStore += parseFloat(InvQuantity);
					   nlapiLogExecution('DEBUG', 'Acct', 'condition 4 =:tempStore='+tempStore);
				   }*/
...
function createRevInventoryTransfer(status,salesOrder,customer,subsidiary,department,Sales_class,aaria_Context,sales_emp,salesApproval_Status,customer_category,toLocation,itemCount,SO_ItemArray,SO_LineLocationArray,descriptionArray,unitsArray,qtyArray,reamainingArray,litersArray,setLocFrom,invDetIdArray)
{
	...
	if(((status == 'A') || (status == 'B') || (status == 'C') ||(salesApproval_Status == '10')) && customer_category == '5')
	{
		...
		for(x=1;x<=SO_ItemArray.length;x++)
		{
			...
			/*
				   if(parseFloat(qtyArray[x-1]) <= parseFloat(InvQuantity) && tempStore == 0)
					{
					   afterQTy = parseFloat(InvQuantity)-parseFloat(qtyArray[x-1]);
					   tempStore += parseFloat(qtyArray[x-1]);
					}
				   else if(parseFloat(qtyArray[x-1]) > parseFloat(InvQuantity) && tempStore == 0)
				   {
				   ...
				   else if(parseFloat(qtyArray[x-1] > InvQuantity) && (tempStore > 0) && (InvQuantity > parseFloat(qtyArray[x-1]-parseFloat(tempStore))))
				   {
					   InvQuantity = parseFloat(qtyArray[x-1])-parseFloat(tempStore);
					   afterQTy = parseFloat(qtyArray[x-1])-parseFloat(tempStore);
					   tempStore += parseFloat(InvQuantity);
				   }
				   else if((qtyArray[x-1] > InvQuantity) && (tempStore > 0) && (InvQuantity < parseFloat(qtyArray[x-1]-parseFloat(tempStore))))
				   {
					   InvQuantity = parseFloat(InvQuantity);
					   tempStore += parseFloat(InvQuantity);
					   afterQTy =0;

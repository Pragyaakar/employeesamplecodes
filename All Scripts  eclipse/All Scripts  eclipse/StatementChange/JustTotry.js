		      ...
		      var BalTotal2 = parseFloat(BalTotal1) + parseFloat( openBalanceWithPayMent );
				...
				//-----------------------------Fetch Values-------------------------------------------
				 for(var l=0;l<jsonArray5.length;l++)
				 {
					...
					// nlapiLogExecution('DEBUG', 'aftr submit', "  newArray== && jsonArray5[l].tranid==" +newArray +'=='+jsonArray5[l].tranid);    
					//  nlapiLogExecution('DEBUG', 'aftr submit', "  newArray== && jsonArray5[l].tranid==" +newArray1 +'=='+jsonArray5[l].tranid_apply);
					// nlapiLogExecution('DEBUG', 'aftr submit', "  newArray == jsonArray5[l].tranid ==" + newArray +'--'+jsonArray5[l].tranid );
					//  nlapiLogExecution('DEBUG', 'aftr submit', " newArray1 == jsonArray5[l].tranid_apply==" + newArray1 +'--'+ jsonArray5[l].tranid_apply);
					 
					               
					               /* if(newArray == doc &&  EntityArr.indexOf(jsonArray5[l].tranid_apply) > -1) // tranIdArr.indexOf(jsonArray5[l].tranid_apply) > -1
					                 {
					                	  zero++;
					                	  nlapiLogExecution('DEBUG', 'aftr submit', "  newArray == doc ==" +newArray +'<->'+ doc ); 
					                	 
					                	 if(zero == 1 && jsonArray5[l].type == 'Journal')
					                	 {
					                		 continue; 
					                	 }

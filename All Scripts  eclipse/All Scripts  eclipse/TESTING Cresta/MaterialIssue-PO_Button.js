		...
		var remPoArr=[];
          ...
          var MaterialRemQtyArr=[];
			...
			for(var i=1;i<=PRlinecount;i++)
			{
              ...
              var invCreated = irObj.getLineItemValue('item','custcol_onlocation_available',i);
				// nlapiLogExecution('DEBUG', 'aftr submit', "  ISrequireQty  ==" + ISrequireQty);
              var MaterialRemQty = irObj.getLineItemValue('item','custcol_material_issue_remain',i);
              if(MaterialRemQty!=null && MaterialRemQty!='' && MaterialRemQty!=undefined && MaterialRemQty!='0' )
				 {
					MaterialRemQtyArr.push(MaterialRemQty);
				 }
              
				if(remQty!=null && remQty!='' && remQty!=undefined && remQty !='0')
				 {
				   remQtyArr.push(remQty);
				
				 }
              
              if(invCreated!=null && invCreated!='' && invCreated !=undefined &&  invCreated !='0')
				 {
				   InvAdArr.push(invCreated);
				...
				/*	 var columns = new Array();
				    ...
				    if(ISrequireQty!=null && ISrequireQty!='' && ISrequireQty!=undefined && ISrequireQty!='0')
					 {
			 ...
			 if( MaterialRemQtyArr.length >=1) //status == '2' &&  commented for pradnyas testing
	         {
		  		    form.setScript('customscript_client_location_acc_redirec');
		  			form.addButton('custpage_button1','Material Issue','clientScriptMIredirect();');
	         }
          else if( InvAdArr.length ==0 || MaterialRemQtyArr.length ==0)
           {
                   form.removeButton('custpage_button1');
            }
	}
	catch(e)
	{

...
function scheduled_to_reverseVendorBal(type) 
{
	...
	//if(parseInt(day) === parseInt(Day) && time == '23:55')  //TIMEZONE
	{
	 ...
	 var searchId='customsearch_vend_with_neg_balance';
			...
			for(var i1=0;i1<resultSet1.length;i1++)//resultSet.length
				{
					...
					var inteId1 = resultSet1[i1].getValue(columns1[0]);
					  ...
					  var subsidiary1 = resultSet1[i1].getValue(columns1[3]);
				    
				     test_array.push(subsidiary1);
				}
		}
	 test_array1  = filter_array(test_array);
	 UniquVendor =removeDuplicates(test_array1);
			...
			if(resultSet!='' && resultSet!= null && resultSet !='undefined')
			{
				...
				nlapiLogExecution('DEBUG','resultSet length','UniquVendor='+UniquVendor);	 
				
				for(var ch=0;ch<UniquVendor.length;ch++)
				{
					try{
					 var subsidiaryArr =[];
					 var custIdArr=[];
					...
					for(var i=0;i<resultSet.length;i++)//resultSet.length
					{
						...
						var custId= resultSet[i].getValue(columns[5]);
						
					    var vendor = resultSet[i].getValue(columns[0]);
					    
					    var NegativAmt = resultSet[i].getValue(columns[1]);
					    
					    var AmountForeign = resultSet[i].getValue(columns[2]);
					    
					    var subsidiary = resultSet[i].getValue(columns[3]);
					    
					    var payAcc = resultSet[i].getValue(columns[4]);
					    
					    nlapiLogExecution('DEBUG','internalId','custId='+custId);
						nlapiLogExecution('DEBUG','internalId','vendor='+vendor);
						nlapiLogExecution('DEBUG','internalId','NegativAmt='+NegativAmt);
						nlapiLogExecution('DEBUG','internalId','subsidiary='+subsidiary);
						nlapiLogExecution('DEBUG','internalId','payAcc='+payAcc);
					    
					    if(UniquVendor[ch] == subsidiary)
						{
						    custIdArr.push(custId);
						    NegativAmtArr.push(NegativAmt);
						    subsidiaryArr.push(subsidiary);
						    payAccArr.push(payAcc);
						}
					...
					/*	if(UniquCustomer[ch] == custId)
					   ...
					   nlapiLogExecution('DEBUG','internalId','custIdArr='+custIdArr);
						...
						nlapiLogExecution('DEBUG','internalId','payAccArr='+payAccArr);
			                
					CreateJVforCustomerNegative(subsidiaryArr,NegativAmtArr,TodayIs,tomorrow,custIdArr,payAccArr);
				}
				catch(e)
				{
...
function CreateJVforCustomerNegative(subsidiaryArr,NegativAmtArr,TodayIs,tomorrow,custIdArr,payAccArr)
{
	...
	jeRec.setFieldValue('reversaldate',tomorrow);
		...
		for (var s=1;s<=subsidiaryArr.length;s++)
		{
			   jeRec.selectNewLineItem('line');
				jeRec.setCurrentLineItemValue('line', 'account',1570);
			    ...
			    jeRec.selectNewLineItem('line');
				jeRec.setCurrentLineItemValue('line', 'account',payAccArr[s-1]);

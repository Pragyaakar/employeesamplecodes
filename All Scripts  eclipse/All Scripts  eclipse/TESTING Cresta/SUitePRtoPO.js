	   ...
	   var sub= form.addField('custpage_subsidiary', 'text', 'subsidiary');
		...
		var vendorName = form.addField('custpage_vendorname', 'select', 'Vendor','vendor');
          ...
          ItemSublist.addField('custpage_chechbox','checkbox');  
		   ...
		   ItemSublist.addField('custpage_lineid', 'integer', 'Line');
            ...
            filters[1] = new nlobjSearchFilter('custcol_remain_qty_for_pr', null, 'greaterthan', '0');
		...
		//	 columns[4] =  new nlobjSearchColumn("altname","vendor",null);
			 ...
			 columns[13] = new nlobjSearchColumn('custcol_auto_line_num');
		...
		{ 
			...
			//	 nlapiLogExecution('DEBUG', 'OrderRequisition', 'unit :'+results[i-1].getValue('unit'));
				 ...
				 ItemSublist.setLineItemValue('custpage_lineid', i, results[i-1].getValue('custcol_auto_line_num'));
	...
	else
	{
		if (request.getMethod() == 'POST') 
		{
		 ...
		 nlapiLogExecution('DEBUG', 'OrderRequisition', 'count :'+count);
          ...
          var lastPurchaseArr = new Array();
          var LineidArr=[];//lastPurchaseArr
    			...
    			// nlapiLogExecution('DEBUG', 'OrderRequisition', 'quantity :'+quantity);
    			 quanArr.push(quantity);
    				...
    				// nlapiLogExecution('DEBUG', 'OrderRequisition', 'unit :'+unit);
    				 ...
    				 var Lineid = request.getLineItemValue('custpage_sig_req_sublist','custpage_lineid',i);
   				  nlapiLogExecution('DEBUG', 'OrderRequisition', 'Lineid :'+Lineid);
		  ...
		  createPurchaseOrderForPR(lineNum,Entity,amtArr,subsiArr,lastPurchaseArr,descripArr,classArr,departArr,LocArr,itemArr,quanArr,unitArr,tranidArr,internalId,LineidArr);
		  response.writePage( form );
		 }
	}
}
...
function createPurchaseOrderForPR(lineNum,Entity,amtArr,subsiArr,lastPurchaseArr,descripArr,classArr,departArr,LocArr,itemArr,quanArr,unitArr,tranidArr,internalId,LineidArr)
{	
  ...
  /*		if(date != '' && date != 'undefined' && date != null)
   ...
   record.setFieldValue('custbody1','1');
	...
	for(var l=1;l<=lineNum;l++)
	{
	        ...
	        var Submitdone = SetQtyAvailable(quanArr,internalId,itemArr,LineidArr);
...
function SetQtyAvailable(quanArr,internalId,itemArr,LineidArr)
{
	...
	for(var w =0;w<itemArr.length;w++)
	{
		for(var r=1;r<=reqCount;r++)
		{
			...
			var ReqLine = loadRequisition.getLineItemValue('item','custcol_auto_line_num',r);
			
			if(remainQTY !=null && remainQTY!=undefined && remainQTY !='')
			{
				var valueUpdate = parseFloat(remainQTY)-parseFloat(quanArr[w]);
				
				if(valueUpdate !=null && valueUpdate!=undefined && valueUpdate!='')
				{
			...
			loadRequisition.setFieldValue('custbody_requisition_status',2);
			
			nlapiLogExecution("DEBUG","In Create Function","itemArr == ReqItemArr "+itemArr+"=="+ReqItem);
			if(LineidArr[w] == ReqLine)
			{
				loadRequisition.setLineItemValue('item','custcol_onlocation_available',r,quanArr[w]);

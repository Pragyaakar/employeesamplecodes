	...
	if(request.getMethod() == 'GET')
	{
     ...
     //  AccountField.setMandatory(true);
		...
		//	 columns[4] =  new nlobjSearchColumn("altname","vendor",null);
			 ...
			 columns[18] = new nlobjSearchColumn('custcol_material_issue_remain');
		...
		{ 
			...
			// nlapiLogExecution('DEBUG', 'OrderRequisition', 'item :'+results[i-1].getValue('item'));
             ...
             var newPr =parseFloat(results[i-1].getValue('quantity'));
             
             var issueRem =parseFloat(results[i-1].getValue('custcol_material_issue_remain'));
             
             if(issueRem !=null && issueRem!='' && issueRem!=undefined)
             {
			 ...
			 ItemSublist.setLineItemValue('custpage_quantity', i, issueRem);
		    ...
		    for(var i=1; i< count+1; i++)
			  {
			   ...
			   if(checkMark == 'T')
			   {
				   ...
				   if(itemArray !=null && itemArray !=''  && itemArray !=undefined )
					{
					 var adjustId =  createInvAdjustViaPR(recId,IsLotItemArr,date,locForm,accForm,vendArr,amtArr,line_chkArr,Class,Department,location,rateArray,recId,itemArray,qtyArray,units,descriptionArray,subsidiary,typeItemArr,useBinsArr);
						 ...
						 for(var k=0;k<itemArray.length;k++)
						   {
							 nlapiLogExecution('DEBUG', 'aftr submit', "  INSIDE PR Set  ==itemArray=" +itemArray);
							   for(var j=1;j<=PRlinecount;j++)
								{
									nlapiLogExecution('DEBUG', 'aftr submit', "  INSIDE PR Set  =PRlinecount=" +PRlinecount);
								      var itemid =recObj.getLineItemValue('item','item',j);
								   
								      var numbOfLine =recObj.getLineItemValue('item','custcol_auto_line_num',j);
								      ...
								      if(itemid ==itemArray[k] && numbOfLine == line_chkArr[k] ) //remainQtyArr
									   {
								    	  ...
								    	  nlapiLogExecution('DEBUG', 'aftr submit', "  INSIDE PR Set  =QtyRem=" +QtyRem);
										    
								    	  recObj.setLineItemValue('item','custcol_material_issue_remain',j,QtyRem);//custcol_remain_qty_for_pr
									   }
								}
						   }
				   ...
				   recObj.setFieldValue('custbody_requisition_status',2);

	 ...
	 if(name == 'custentity_cntry')
	    {
			...
			var phneCode2 = new Array();
		    	...
		    	if (searchResultItem2 != null)
			   {
				...
				var concat="+"+MobCode2[0]+phneCode2[0];
    ...
    if(name == 'custentity_mobile')
    {
   	...
   	//	alert('country Id is--'+Id);
		...
		if (searchResultItem2 != null)
		{
			...
			if(mobstrnlength != p1 )
			{
				alert('NOT allowed ... You have entered Invalid Length which is: '+mobstrnlength+' ..Please Enter Allowed Mobile Length for your country Is : '+p1+' ..Do not delete the country code...Give the proper Area code like:'+concat+','+"+"+MobCode2[1]+phneCode2[1]);
   	    	...
   	    	if(mobNo)
   	    	{
   				...
   				else
   				{
   					if(Id != 'CA')
   					{
   						var mobileId=['AF','AR','AU','AT','BE','BR','IC','CL','CX','CC','CO','CU','DK','EG','FR','DE','GR','GG','HU','IN','ID','IM','IT','JP','JE','MY','MX','MM','NL','NZ','NO','PK','PE','PH','PL','RO','SG','ZA','ES','LK','SE','CH','TH','TR','GB','VE','VN',];
...
function findMatchforMobandPhone(Id,code,length,mobNo,concat)
{
	...
	if (searchResultItem != null)
	{
		...
		alert(' Mobile Number is Valid ...Stored In record.')
...
function findPhoneAndMobile(country,phoneCode,mobPhneCode,phoneLngth,mobstrnlength,mobNo,phone)
{
	...
	if (searchResultItem != null)
	{
		for(var k=0;k<searchResultItem.length;k++)
		{
			...
			{
				nlapiSetFieldValue('custentity_mobile',MobCode);
			}
			if(phone)
			{

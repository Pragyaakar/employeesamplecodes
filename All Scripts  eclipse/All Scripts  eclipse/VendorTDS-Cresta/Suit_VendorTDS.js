	 ...
	 * 1.00       16 May 2019           		   Display all click
	 * 1.00       22 May 2019           		   Set Tax Amount Value - add formula (Tax Basis * Tax Rate) /100   
	 * 1.00       22 May 2019     		           Tax Details Override Checkbox will be check at time of submitting.
	 * 1.00       22 May 2019           		   Add Column in Sublist - Cheque No and Cheque Date.
	 */
	...
	
	{
		   ...
		   if ( request.getMethod() == 'GET' )
		   {
		    ...
		    {    
			   ...
			   var form = nlapiCreateForm('Advance TDS Apply',true);
			   ...
			   sublist.addField('custpage_vendadvdebitamt', 'float', 'Advance Paid').setDisplayType('inline');
			   sublist.addField('custpage_advancepaid', 'float', 'Advance Paid').setDisplayType('hidden');
			  ...
			  // sublist.addField('custpage_vendadvdebitamt', 'float', 'VendorAdvanceDebitAmount').setDisplayType('inline');
			   sublist.addField('custpage_billamount', 'float', 'Bill Amount').setDisplayType('hidden');
			   ...
			   form.setScript("customscript_cls_advancevp");
		      ...
		      // var searchresults = nlapiSearchRecord('transaction','customtransaction102', filters, columns);
		       ...
		       for ( var i = 0; searchresults != null && i < searchresults.length; i++ )   
		       {
			      ...
			      */ 
			       ...
			       var secRate;
			       if(taxRateCheque != null && taxRateCheque != undefined && taxRateCheque != ''){
			    	    secRate = nlapiLookupField('customrecord_in_tds_setup',taxRateCheque,'custrecord_in_tds_setup_rate');
		    	  ...
		    	  */ 
		    	   ...
		    	   nlapiLogExecution('DEBUG', 'BeforeLoadRecord check', "recordId befor bill==" + recordId);
		    	   
		    	
		    	   var billAmount = nlapiLookupField('vendorbill',recordId,'total');
			       nlapiLogExecution('DEBUG', 'BeforeLoadRecord check', "billAmount" + billAmount);
		    	  ...
		    	  sublist.setLineItemValue('custpage_billamount',i+1,billAmount);
			   	...
			   	if (request.getMethod() == 'POST') 
				{
			   	...
			   	//	 var lineNum =0;
					  ...
					  for(var i=1; i<count+1; i++)
					  {
					   ...
					   var billId = addValuetoFuction(recordId,recordType,accountP,ampuntP,departmentP,classPid,locationP,count,grossAmt,internalid,checkMark,taxcd,rateTaxP,classPostid);
					...
					//START : if TDS already applied
					nlapiSubmitField('vendorbill',recordId,'custbody_advance_tds_applied','T');				
					//END   : if TDS already applied  
	              ...
	              // form.close();;
					 ...
					 /* var date = new Date;
					  var vendPaymntObj = nlapiCreateRecord('vendorpayment', {recordmode: 'dynamic'});
					   	...
					   	vendPaymntObj.setFieldValue('class',classPostid[0]);
					   	...
					   	var lineNum = nlapiFindLineItemValue('apply', 'internalid', recordId);
					   	vendPaymntObj.selectLineItem('apply', lineNum);
					   	vendPaymntObj.setCurrentLineItemValue('apply', 'apply', 'T');
					   	vendPaymntObj.setCurrentLineItemValue('apply', 'amount',billamt);
					   	nlapiLogExecution("DEBUG","refnum","Bill amount Set ");
					   	vendPaymntObj.commitLineItem('apply');			   	
					  
					   		for(var k=0;k<vendAdvnArray.length;k++)
					   		{
					   			nlapiLogExecution("DEBUG","refnum","vendAdvnArray[k] "+vendAdvnArray[k]);
					   			var lineNum = nlapiFindLineItemValue('apply','internalid', vendAdvnArray[k]);
					   			vendPaymntObj.selectLineItem('apply',lineNum);
					   			vendPaymntObj.setCurrentLineItemValue('apply', 'apply', 'T');
					   			vendPaymntObj.setCurrentLineItemValue('apply', 'amount',debAmtVendAdv[k]);
					   			nlapiLogExecution("DEBUG","refnum","Debit amount Set ");
					   			vendPaymntObj.commitLineItem('apply');
					   		}
	...
	function addValuetoFuction(recordId,recordType,accountP,ampuntP,departmentP,classPid,locationP,count,grossAmt,internalidP,checkMark,taxcd,rateTaxP,classPostid)
	{
		...
		if(rateTaxP != null && rateTaxP != undefined && rateTaxP != '' && parseFloat(rateTaxP) != 0.0 && isNaN(parseFloat(rateTaxP))!=true){

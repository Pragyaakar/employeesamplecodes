	function schedule_GetData_deploy(type)
	{
		var myCSV_data = nlapiRequestURL('https://3572894.app.netsuite.com/core/media/media.nl?id=498754&c=3572894&h=477e0a221fb9ffc8521c&_xt=.json',null,null);
		...
		// nlapiLogExecution('DEBUG','postFunction',' *****string_data***** -->'+datain)
		 ...
		 try
		 {
			 ...
			 var PinCounter = '';
			 PinCounter = context.getSetting('SCRIPT', 'custscript_new_pincounter');
			...
			if(type.trim()=='Invoice' || type.trim()=='Return Authorization')
			{
				...
				record.setFieldValue('custbody_old_transaction_int_id',recId);
			    	...
			    	/* else if( NewCount > 0)
			    	 {
				    		...
				    		loop2: for(var ch=1;ch<=count;ch++)
					    	 {
					    		 ...
					    		 var ratel = record.getLineItemValue('item','rate',ch);
					    		  ...
					    		  if(CheckItemArr.indexOf(ItemLine) != -1)
					    		  {
					    			 // nlapiLogExecution("DEBUG","In Create Function","ItemLine="+ItemLine+' -[Rate Condition]='+(ratel ==rateArr[m]));
					    			 // nlapiLogExecution("DEBUG","In Create Function","CheckItemArr[m]="+CheckItemArr[m]+' -[QTy Condition]='+(qtty == qtyArr[m]));
											
					    			  
					    			  if((ItemLine == CheckItemArr[m])&&((parseFloat(ratel)==parseFloat(rateArr[m]))||(parseFloat(qtty)==parseFloat(qtyArr[m]))) && (setValue != 'T'))
							    		 {
							    			 nlapiLogExecution("DEBUG","Condition True","ItemLine="+ItemLine+' -[setValue]='+setValue);
			    		 ...
			    		 for(var h=count;h>=1;h--)
					 		{
                              var ItemLine1 = record.getLineItemValue('item','item',h);
				    		 
				    		   var setField = record.getLineItemValue('item','custcol_set_value',h);
				 ...
				 //  nlapiLogExecution("DEBUG","In Create Function","DocToCheck.trim()="+DocToCheck.trim());
				   ...
				   /* if(SubmitIt != null && SubmitIt != undefined)
					   {
						   var Obj =nlapiLoadRecord('creditmemo',SubmitIt);
							    ...
							    for( var ah=1;ah<=CheckItemArr.length;ah++)
								   {
							    	 ...
							    	 if(CheckItemArr[ah-1] != avlItem)
							    	 {
							    		 ...
							    		 if(CheckItemArr[ah-1] != null && CheckItemArr[ah-1] != undefined && CheckItemArr[ah-1] != '')
								    	  ...
								    	  if(InvDetailNumArr[ah-1] != null && InvDetailNumArr[ah-1] != undefined && InvDetailNumArr[ah-1] != '')
								    		{
								    		  Obj.setCurrentLineItemValue('item','serialnumbers',InvDetailNumArr[ah-1]);
								    		}
	...
	function Schedulescriptafterusageexceeded(i)
	{
		//Define all parameters to schedule the script for voucher generation.
		 ...
		 // params['status']='scheduled';
	 	 // params['runasadmin']='T';
		 params['custscript_new_pincounter']=i;

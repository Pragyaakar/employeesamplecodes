	...
	{
		var myCSV_data = nlapiRequestURL('https://3572894.app.netsuite.com/core/media/media.nl?id=489658&c=3572894&h=74bba7b03794b17055a5&_xt=.json',null,null);
		...
		var context = nlapiGetContext();
		 try
		 {
			...
			for(var i=PinCounter;i<datain.length;i++)//datain.length
			{
						...
						if(Location != null && Location != '' && Location !=undefined)
						 {
						  ...
						  createCustomerPaymentTrans(recId,AR_AccName,AccName,Payee,TranNum,CheckNum,Memo,Department,Location,DueDateArr
								  ,refNumArr,OrigAmtArr,AmtDueArr,DiscDateArr,DiscAvailArr,DiscTakeArr,PaymentArr,tranDate,DepositLineRefArr,DepositLineAmtArr,CredLineRefArr,CredLineAmtArr,PayMethod,pnref,cardNumber,cardExpiry,cardName,cardProcessor,PayMent,DateCreated);
					  ...
					  var ExpAmtArr = checkData['record_exp_line_amt'];
						...
						if(location != null && location != '' && location !=undefined)
					     ...
					     createIRfunction(AccountName,recId,type,tranDate,docNum,Vendor,department,location,itemArr,qtyArr,
								   lineDeptArr,lineClassArr,lineLocArr,lineCustArr,unitsArr,descripArr,vendNameArr,
								   rateArr,amtArr,AccArr,ExpAmtArr,ExplineDeptArr,ExplineClassArr,ExplineLocArr,ExplineCustArr,InvDetailNumArr,InvDetailQtyArr,createdFrom,DateCreated);
					  ...
					  var ExpAmtArr = checkData['record_exp_line_amt'];
						...
						if(location != null && location != '' && location !=undefined)
					     ...
					     createIFfunction(AccountName,recId,type,tranDate,docNum,Vendor,department,location,itemArr,qtyArr,
								   lineDeptArr,lineClassArr,lineLocArr,lineCustArr,unitsArr,descripArr,vendNameArr,
								   rateArr,amtArr,AccArr,ExpAmtArr,ExplineDeptArr,ExplineClassArr,ExplineLocArr,ExplineCustArr,InvDetailNumArr,InvDetailQtyArr,createdFrom,shipStatus,DateCreated);
				...
				{
					var custRec = nlapiCreateRecord('customrecord_issue_rec_integ',{recordmode: 'dynamic'});
	...
	function createInvoiceFunction(recId,docNum,customer,tranDate,department,location,salesEffDate,shipDate,shipCarrier,actulShipDate,exchRate,estExtCost,estGrossProfit,estGrossProfitPercent,itemArr,qtyArr,qtyCommitArr,qtyFulfillArr,qtyInvoicedArr,qtyBackOrderArr,unitsArr,descripArr,priceLvlArr,rateArr,amtArr,costEstTypeArr,TaxItem,InvDetailNumArr,InvDetailQtyArr,createdFrom,shipTerm,DateCreated)
	{
		...
		if(createdFrom != null && createdFrom != '' && createdFrom != undefined)
		{
					   ...
					   if(itemArr[l] != null && itemArr[l] != undefined && itemArr[l] != '')
						{
						  ...
						  // var rateIs =0.00;
						   ...
						   qtyMap[itemSet]=qtyArr[l];
						   rateMap[itemSet]=rateIs;
						   serialMap[itemSet]=InvDetailNumArr[l];
		   ...
		   //  nlapiLogExecution('DEBUG', 'After Submit item', "  linecount==" + linecount);
		     if(linecount > 0)
		     {
		   	  for(var h=linecount;h>=1;h--)
		   	  {
		   		 var item = record.getLineItemValue('item','item',h);
		   		  
		   		 var rate = record.getLineItemValue('item','rate',h);
					//	nlapiLogExecution('DEBUG', 'After Submit item', "  h==" + h);
						 
						   if(CheckItemArr != null && CheckItemArr != undefined)
								{
							 //  nlapiLogExecution('DEBUG', 'After Submit item', "  item==" + item +'  CheckItemArr='+CheckItemArr);
							 //  nlapiLogExecution('DEBUG', 'After Submit item', "  CheckItemArr.indexOf(item)==" + CheckItemArr.indexOf(item));
							 //  nlapiLogExecution('DEBUG', 'After Submit item', "  rate==" + rate +'  CheckRateArr='+CheckRateArr);
							 //  nlapiLogExecution('DEBUG', 'After Submit item', "  CheckRateArr.indexOf(rate)==" + CheckRateArr.indexOf(rate));
							       if(CheckItemArr.indexOf(item) != -1 && CheckRateArr.indexOf(rate) != -1)
							       {
							    	  // record.setLineItemValue('item','itemreceive',h,'T');
										 record.setLineItemValue('item','quantity',h,qtyMap[item]);
										 record.setLineItemValue('item','rate',h,rateMap[item]);
			    	  ...
			    	  for(var h=1;h<=linecount;h++)
			    	  {
			    		  item = record.getLineItemValue('item','item',h);
							nlapiLogExecution('DEBUG', 'After Submit item', "  item==" + item);
							   ...
							   if(CheckItemArr != null && CheckItemArr != undefined)
									{
								       if(CheckItemArr.indexOf(item) != -1)
								       {
								    	   record.setLineItemValue('item','itemreceive',h,'T');
											 record.setLineItemValue('item','quantity',h,qtyMap[item]);
											 
											 if(serialMap[item] != null && serialMap[item] != undefined)
									  	      {
												 record.setLineItemValue('item','serialnumbers',h,serialMap[item]);
									  	      }
								       }
								       else
								       {

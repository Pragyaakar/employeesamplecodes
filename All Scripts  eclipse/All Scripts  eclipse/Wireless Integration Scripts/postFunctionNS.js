...
function postFunction_SXP_Netsuite(datain)
{	
 try
 {
	 var s_result;
	...
	for(var i =0;i<datain.length;i++)
	{
		...
		if(type =='SalesOrd' || type =='SalesOrd')
		{
		...
		var subsi = checkData['record_subsi'];
       ...
       // nlapiLogExecution('DEBUG','postFunction_SXP_Netsuite',' customer In -->'+customer)
        ...
        subsi = FindSubsidiary(subsi.toString());
...
/*        
		...
		*/
            createSalesOrderFunction(createdFrom,docNum,customer,tranDate,subsi,location,salesEffDate,shipDate,shipCarrier,actulShipDate,currency,exchRate,estExtCost,estGrossProfit,estGrossProfitPercent,itemArr,qtyArr,qtyCommitArr,qtyFulfillArr,qtyInvoicedArr,qtyBackOrderArr,unitsArr,descripArr,priceLvlArr,rateArr,amtArr,costEstTypeArr);
		}
...
function createSalesOrderFunction(createdFrom,docNum,customer,tranDate,subsi,location,salesEffDate,shipDate,shipCarrier,actulShipDate,currency,exchRate,estExtCost,estGrossProfit,estGrossProfitPercent,itemArr,qtyArr,qtyCommitArr,qtyFulfillArr,qtyInvoicedArr,qtyBackOrderArr,unitsArr,descripArr,priceLvlArr,rateArr,amtArr,costEstTypeArr)
{
	...
	var record = nlapiCreateRecord('salesorder', {recordmode: 'dynamic'}); 
	   ...
	   record.setFieldValue('subsidiary',subsi);
...
{ 
	...
	if(subsi != null && subsi != '')
    {
	 filters[0] = new nlobjSearchFilter('name', null,'contains',subsi);
	}
	
	columns[0] = new nlobjSearchColumn('internalid');
	columns[1] = new nlobjSearchColumn('name');
	
	var searchResultItem = nlapiSearchRecord('subsidiary', 'customsearch_inte_subsi_search', filters, columns);

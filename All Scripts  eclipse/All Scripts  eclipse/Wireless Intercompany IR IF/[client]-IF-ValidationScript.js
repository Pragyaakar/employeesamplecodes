...
function clientSaveRec_SUT_IF()
{
	...
	for(var k=1;k<=linecount;k++)
	{
		...
		var itemsorqty= nlapiGetLineItemValue('custpage_item_sublist','custpage_qty_so',k);
		...
		{	
			...
			if(parseFloat(itementerqty) > parseFloat(itemsorqty))
			{
				//alert('if part');
...
function clientFieldChng_SUT_IF(type, name, linenum)
{
	...
	if(type=='custpage_item_sublist' && name=='custpage_qty')
	{
		...
		var so_qty = nlapiGetLineItemValue('custpage_item_sublist','custpage_qty_so',linenum);
		//alert('so_qty '+so_qty);
      ...
      //   alert('quantity qty='+quantity)
		...
		else if(parseFloat(quantity) > parseFloat(search_qty))
		{
			...
			alert('The number of serial numbers entered ('+quantity+') is not equal to the item On Hand quantity ('+search_qty+')');
			nlapiSetLineItemValue('custpage_item_sublist','custpage_qty',linenum,so_qty,false,false);
			check=false;
		}	

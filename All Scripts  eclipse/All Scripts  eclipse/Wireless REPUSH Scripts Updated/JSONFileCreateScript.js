				 ...
				 for(var i=0;i<resultSet.length;i++)//resultSet.length
					{
						try
						{
							...
							var date = resultSet[i].getValue(columns[4]);
                            var totTranAmt = resultSet[i].getValue(columns[5]);
                            var DateCreated = resultSet[i].getValue(columns[1]);
							var createdFrom =resultSet[i].getValue(columns[6]);
							
							if(createdFrom =='- None -'){
								createdFrom=null;
							}
							// nlapiLogExecution("DEBUG","In Create Function","createdFrom =="+createdFrom);
					//===============================================================		
						    if(type =='CustInvc' || type =='CashSale'|| type =='SalesOrd' || type =='RtnAuth' || type =='Estimate') //CustInvc ..SalesOrd,RtnAuth
							{
							  ...
							  if(type =='CustInvc')
						      {
						       ...
						       {
						    	  var loadRec = nlapiLoadRecord('returnauthorization',recId);
						       } 
							  if(loadRec != null && loadRec != '' && loadRec !=undefined)
							  {
								 ...
								 var shipCost = loadRec.getFieldValue('shippingcost');
								  var shipCarrier = loadRec.getFieldValue('shipcarrier');
								  var actulShipDate = loadRec.getFieldValue('actualshipdate');
								  var shipTerm =loadRec.getFieldValue('custbody_es_shipping_terms');
                                ...
                                var shipMethod = loadRec.getFieldValue('shipmethod');
                                var DiscItem = loadRec.getFieldValue('discountitem');
                                var DiscRate = loadRec.getFieldValue('discountrate');
								  ...
								  */
								  ...
								  var lineCount = loadRec.getLineItemCount('item');
								    ...
								    for( var l=1;l<=lineCount;l++)
									{
									    ...
									    var serialnum = loadRec.getLineItemValue('item','serialnumbers',l);
									    ...
									    InvDetailNumArr.push(serialnum);
									...
									transMap['record_shipterm']=shipTerm;
                                
									transMap['record_shipcost']=shipCost;
	                                transMap['record_shipmethod']=shipMethod;
	                                transMap['record_discnt_item']=DiscItem;
	                                transMap['record_discnt_rate']=DiscRate;
									...
									transMap['record_datecreated']=DateCreated;
						 ...
						 else if(type =='CustCred')
						 {
							  ...
							  if(loadRec != null && loadRec != '' && loadRec !=undefined)
							  {
								 ...
								 // var createdFrom = loadRec.getFieldText('createdfrom');
								  ...
								  var location = loadRec.getFieldValue('location');
								  var AccountName = loadRec.getFieldValue('account');
								   var shippTerms = loadRec.getFieldValue('custbody_es_shipping_terms');
								  ...
								  if(lineCount2 > 0)
								  {
									...
									transMap['record_itmarr']=itemArr;
									transMap['record_qtyarr']=qtyArr;
									transMap['record_shipterms']=shippTerms;
									transMap['record_item_line_dept']=lineDeptArr;
									transMap['record_item_line_clss']=lineClassArr;
									transMap['record_item_line_loc']=lineLocArr;
									transMap['record_item_line_cust']=lineCustArr;
					...
					//==================================================================
						 ...
						 else if(type =='CustPymt'||type =='CustRfnd')
						 {
					        ...
					        else if(type =='CustRfnd')
					        {
							 ...
							 var CheckNumber =loadRec.getFieldValue('checknum');
							 ...
							 var Location =loadRec.getFieldValue('location');//payment
							 
							 var PayMent =loadRec.getFieldValue('payment');//payment
							  ...
							  if(lineCount2 > 0)
							  {
								...
								transMap['record_deptmt']=Department;
								transMap['record_location']=Location;
								transMap['record_refnumber']=CheckNumber;
							  ...
							  if(loadRec != null && loadRec != '' && loadRec !=undefined)
							  {
								...
								//  var createdFrom = loadRec.getFieldText('createdfrom');
									  ...
									  for( var l=1;l<=lineCount;l++)
										{
										    ...
										    var line = loadRec.getLineItemValue('item','line',l); 
								  ...
								  if(lineCount1 > 0)
								  {
									...
									transMap['record_unit_arr']=unitsArr;
									transMap['record_descrp_arr']=descripArr;
									transMap['record_line_arr']=LineArr;
					...
					//==================================================================
						 ...
						 else if(type=='InvAdjst')
						 {
							 ...
							 var loadRec = nlapiLoadRecord('inventoryadjustment',recId);
							 
							 var AccName= loadRec.getFieldValue('account');
							 
							 var employee =loadRec.getFieldValue('customer');
							 ...
							 var Location =loadRec.getFieldValue('adjlocation');
							 
							  var lineCount = loadRec.getLineItemCount('inventory');
							  ...
							  if(lineCount > 0)
							  {
								  for( var l=1;l<=lineCount;l++)
									{
									    var item = loadRec.getLineItemValue('inventory','item',l);
									 
									    var desc = loadRec.getLineItemText('inventory','description',l);
									    
									    var rate = loadRec.getLineItemValue('inventory','unitcost',l);
									    
									    var qty = loadRec.getLineItemValue('inventory','adjustqtyby',l);
									    
									    var memo = loadRec.getLineItemValue('inventory','memo',l);
									    
									    var serial = loadRec.getLineItemValue('inventory','serialnumbers',l);
										
									    var lineLoc = loadRec.getLineItemValue('inventory','location',l);
									    
									    itemArr.push(Date);
									    descripArr.push(desc);
									    rateArr.push(rate);
									    qtyArr.push(qty);
										memoArr.push(memo);
								...
								transMap['record_line_item']=itemArr;
								transMap['record_line_desc']=descripArr;
								transMap['record_line_rate']=rateArr;
								transMap['record_line_qty']=qtyArr;
							    transMap['record_line_memo']=memoArr;
								transMap['record_line_serial']=serialArr;
								transMap['record_line_location']=locArr;
								transMap['record_trans_total']=totTranAmt;
								transMap['record_datecreated']=DateCreated;
								s_JSON_Data.push(transMap);
						 ...
						 else if(type=='TrnfrOrd')
						 {
							 ...
							 var loadRec = nlapiLoadRecord('transferorder',recId);
							 ...
							 var Memo =loadRec.getFieldValue('memo');
                              ...
                              var FromLocation =loadRec.getFieldValue('location');
                              
							 var ToLocation =loadRec.getFieldValue('transferlocation');//incoterm
							 
							 var term =loadRec.getFieldValue('incoterm');//incoterm
							  ...
							  if(lineCount > 0)
							  {
								  for( var l=1;l<=lineCount;l++)
									{
									    ...
									    var desc = loadRec.getLineItemText('item','description',l);
									    
									    var rate = loadRec.getLineItemValue('item','rate',l);
									    
									    var qty = loadRec.getLineItemValue('item','quantity',l);
									    
									    var memo = loadRec.getLineItemValue('item','memo',l);
									    
									    var serial = loadRec.getLineItemValue('item','serialnumbers',l);
							    ...
							    transMap['record_memo']=Memo;
								transMap['record_deptmt']=Department;
								transMap['record_term']=term;
								
								transMap['record_location']=FromLocation;
								transMap['record_tolocation']=ToLocation;

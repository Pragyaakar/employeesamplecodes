...
function BeforeSubmitSOforIF(type, form, request)
{
   ...
   for (var m=1;m<=LineCount;m++)
   {
	   ...
	   if(IsBulkItem =='T')
	   {
		   ...
		   var productType = nlapiGetLineItemValue('item','custcol_product_type',m);//,m
		   var KB_TotalOrderLoad = nlapiGetLineItemValue('item','quantity',m);//,m
		   var UnitsOfMeasure = nlapiGetLineItemValue('item','units',m);//,m
...
//================================Find nearest location which having stock===========================================
		    ...
		    var item = nlapiGetLineItemValue('item','item',m);

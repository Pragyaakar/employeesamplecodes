...
*/		
	...
	function recalcAddDelivery(type)
	{
	    if(type == 'item')
	    {
	        ...
	        if(isBulk == 'T' && KB_NumLoad != null && shipIs !=null && rateOn != null ) //Repair Cost
	        {
	                //Insert item
	                nlapiSelectNewLineItem('item');
	                nlapiSetCurrentLineItemValue('item', 'item', 2213); //Repair Cost
	                nlapiSetCurrentLineItemValue('item', 'quantity', KB_NumLoad);
	                nlapiSetCurrentLineItemValue('item', 'rate', parseFloat(rateOn).toFixed(2));
	                //nlapiSetCurrentLineItemValue('item', 'taxcode', '-7');
	                nlapiCommitLineItem('item');
	        }
	    }
	    return true;

	...
	function onSaveValidation(type)
	{
       ...
       var posRegister =nlapiGetFieldValue('custbody_fnxpos_register');
       var customerPriceLvel =nlapiGetFieldValue('custbody_tcspos_price_category');
       
       var ShipLable = nlapiGetFieldValue('shipaddresslist');
     // alert('ShipLable ='+ShipLable)
      if((createFrom == '' || createFrom == null || createFrom == undefined)&&(posRegister == '' || posRegister == null || posRegister == undefined))
      {
	  ...
	  //  alert('Zipcode ='+CustzipCode);
		  ...
		  // alert('IsBulkItem ='+IsBulkItem);
		   var getedPrice =  nlapiGetCurrentLineItemValue('item','price');//custbody_fnxpos_register
           
		   var lineValue = nlapiGetCurrentLineItemIndex('item');
		  // alert('lineValue ='+lineValue);
		   
		   if((IsBulkItem =='T') && (lineValue == '1') && (vehicleType != null && vehicleType != '' && vehicleType != undefined ) && (getedPrice != -1))
		   {
			   isBulkitm = IsBulkItem;
			 ...
			 //  alert('UnitsOfMeasure ='+UnitsOfMeasure);
			    ...
			    if(vehicleType != null && vehicleType !='' && vehicleType != undefined)
				{
			 	...
			 	//==========================================================================================================		    
			 			   ...
			 			   nlapiLogExecution('DEBUG', 'SMallest Location value','MaxCapacityStore ='+MaxCapacityStore);
                        ...
                        var getedPrice =  nlapiGetCurrentLineItemValue('item','price');//custbody_fnxpos_register
                         ...
                         // if(getedPrice != '-1' )
                            ...
                            nlapiSetCurrentLineItemValue('item','price',ValueOfPriceLvl,false,true);
			 		  
                         
                            priceValue=ValueOfPriceLvl;
		    ...
		    else if((IsBulkItem =='F') && (lineValue == '1') && (vehicleType != null && vehicleType != '' && vehicleType != undefined ) && (getedPrice != -1))
 		   {

 			  var KB_TotalOrderLoad;
				//================================Find nearest location which having stock===========================================
	...
	function MaxCapacityFind(vehicleType,productType,UnitsOfMeasure,CustzipCode)
	{
		...
		if (searchResultItem != null && searchResultItem != '' && searchResultItem != undefined)
		{
			for(var j=0;j<searchResultItem.length;j++)
			{
				...
				if(UnitsOfMeasure =='37' || UnitsOfMeasure =='42')
				{
	...
	function cltFieldChangeValidation(type,name,linenum)
	{
		...
		if(type == 'item' && name == 'item')
		{
			 ...
			 var delLoc =nlapiGetFieldValue('location');
		
			 if(delLoc !=null && delLoc !='')
			 {
				 var desc = nlapiGetCurrentLineItemValue('item','description');
			       
					if(linenum > 1 && (priceValue != null && priceValue !=undefined && priceValue !='') && desc != 'Delivery')
					{
					    	//alert('Set Pricelevel='+priceValue)
						 nlapiSetCurrentLineItemValue('item','price',priceValue,false,true);
					}
			 }
	...
	function lneInitKb(type)
	{
		 var item = nlapiGetCurrentLineItemValue('item','item');
      	var qqtyav = nlapiGetCurrentLineItemValue('item','quantityavailable');
      	//var item = nlapiGetCurrentLineItemValue('item','item');
		//alert(' Line init Item qty avail'+qqtyav);
       if(item == null || item == '' || item == undefined)// && (qqtyav != null || qqtyav != '' || qqtyav != undefined)
       {
		   nlapiSetCurrentLineItemValue('item','item','',true,true)
			//nlapiSetCurrentLineItemValue('item', 'quantityavailable', '',false,true);
			//nlapiSetCurrentLineItemValue('item', 'quantityonhand', '',false,true);
	...
	function recalcAddDelivery(type)
	{
	    if(type == 'item')
	    {
          ...
          // alert('linecnt'+linecnt);
           var newLine = nlapiGetCurrentLineItemIndex('item');
          ...
          //	alert('isBulk'+isBulk);
	       ...
	       // if((isBulk == 'T' )&& (lineValue == 1 )&& (KB_NumLoad != null) && (shipIs !=null) && (rateOn != null )&& (getedPrice !=-1)) //Repair Cost
	       if(linecnt == 1 && isBulkitm == 'T')
	        {
	                //Insert item
	                 nlapiSelectNewLineItem('item');
	                nlapiSetCurrentLineItemValue('item', 'item', 8934,false,true); // Previous id ->2213istaxable
	                nlapiSetCurrentLineItemValue('item', 'quantity', KB_NumLoad,false,true);
                    nlapiSetCurrentLineItemValue('item', 'price', '-1',false,true);
	                nlapiSetCurrentLineItemValue('item', 'rate', parseFloat(rateOn).toFixed(2),false,true);
              		nlapiSetCurrentLineItemValue('item', 'istaxable', 'F',false,true);
       ...
       if((item == null || item == '' || item == undefined) && (qqtyav != null || qqtyav != '' || qqtyav != undefined))
       {
		   ...
		   alert('newLine'+newLine);
		   nlapiSelectLineItem('item',newLine)
		   nlapiRemoveLineItem('item',newLine)
	...
	function validateLinePrice(type)
	{
		if(type == 'item' && name == 'item')
		{
			...
			if(priceLevel != null && priceLevel != '' && priceLevel != undefined)
			{
				...
				nlapiSetCurrentLineItemValue('item','custcol_cust_pricelevel_category',priceLevel,false,true);
				flag1 = true;
			}
			else
			{

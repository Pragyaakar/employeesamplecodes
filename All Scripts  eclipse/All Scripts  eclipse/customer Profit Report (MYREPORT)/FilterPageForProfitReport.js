...
function showForm(request,response){
  ...
  if(user == '-5' || user == '12729')
  {
     user = '-5'; //Need to remove before go live (This is for testing)
  }
  else 
  {
    user =user;
	...
	var parameters = '&stdt='+fiscalYear+'&eddt'+today+'&srep='+user;
	url += parameters;
	...
	/*var displayCommissionByFld =  form.addField('custpage_disp_commision','select','DISPLY COMMSSION BY').setBreakType('startcol');;
	
	 var usage = context.getRemainingUsage();
	 nlapiLogExecution('DEBUG', 'Before Usage', usage);
	displayCommissionByFld.addSelectOption(1, 'Sales Rep');
	displayCommissionByFld.addSelectOption(2, 'Customer Name');
	displayCommissionByFld.addSelectOption(3, 'Project');
	displayCommissionByFld.addSelectOption(4, 'Order');
	displayCommissionByFld.addSelectOption(5, 'Detailed',true);
	
	 var usage = context.getRemainingUsage();
	 nlapiLogExecution('DEBUG', 'After Usage', usage);
	
	displayCommissionByFld.setDisplayType('disabled');*/
	//var inlineFLd = form.addField('custpage_all_salesrep','checkbox','All Sales Rep')
